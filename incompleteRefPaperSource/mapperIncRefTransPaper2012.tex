...
\cite{Ye2011}.  However, the computational pipelines to process \rnaseq{} data were developed 
for organisms with well-studied reference transcriptomes.
In this study, 
...
a transcriptome from the \rnaseq{} reads via reference-guided or {\em de novo} assembly 
...
splice variants often share exons, and distinct transcripts can be difficult to resolve 
...
equally well to multiple locations in the transcriptome \cite{Oshlack2010}.  These are usually  caused by alternative splice variants that share exons or repetitive regions or 
recently divergent homologs \cite{Garber2011}.  Some studies have placed the number of multimap reads 
...
the rescue method of using unique reads to initially estimate expression 
...
\cite{Mortazavi2008,Marioni2008,Cloonan2009,Hashimoto2009,Jiang2009,Li2010,Trapnell2010,Li2011,Trapnell2012} 
...
to infer transcript expression from mapped reads (reviewed in \cite{Trapnell2012}).
...
paired end reads were 100 bp with 250 bp insert size.  An error rate of 1\% was simulated by 
...
single-end read lengths between 100 and 250 bp in length, Roche 454 can achieve 400-800 bp, and Pacific Biosciences is 
developing a method that will yield 10000 bp, albeit with lower numbers of reads.  To determine if these lengths are sufficient to increase 
...
using {\em de novo} or reference-based assembly methods.  Our read mapping simulations suggest that 
...
is subject to significant uncertainty \cite{Trapnell2010,Trapnell2012}.  Moreover, this is unlikely to change until technologies that permit 
